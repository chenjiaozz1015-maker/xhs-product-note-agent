...
- 当前 v0.3-1 不实际接入外部引擎，不引入新依赖，不下载外部代码
...
## Render 版本变量提醒
`/health` 返回的 `version` 优先读取 `APP_VERSION` 环境变量，代码默认值为 `v0.3-1`。如果 Render 线上 `/health` 仍显示旧版本，例如 `v0.1-5`，请检查 Render Environment 里的 `APP_VERSION`，改为 `v0.3-1` 或删除该环境变量后重新部署。
